...
int main(void)
{
    ...
    engine.init();
    auto resource = engine.resource();
    auto list = engine.render();
    auto input = engine.input();
    auto font = resource->fonts().load("roboto", "fonts/Roboto/Roboto-Medium.ttf", 64);

    input->bind_key("jump", GLFW_KEY_SPACE);
    input->bind_mouse_button("shoot", GLFW_MOUSE_BUTTON_LEFT);

    while (engine.running)
    {
        engine.begin_frame();

        if (input->is_action_pressed("jump")) LOG_INFO("JUMPP");
        if (input->is_action_pressed("shoot")) LOG_INFO("SHIT");
